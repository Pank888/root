...
var _magicLog2 = _interopRequireDefault(_magicLog);
...
var renderTemplate = exports.renderTemplate = function renderTemplate(res, template) {
  ...
  (0, _magicLog2.default)(libName + ': rendering template ' + template);
  res.render(template, function (err, html) {
    if (err) {
      _magicLog2.default.error('magic-view: error in res.render ' + err);
    }
    if (!html) {
      _magicLog2.default.error(libName + ': html file was empty for template ' + template);
    }
    if (err || !html) {
      return next();
...
var getTemplate = function getTemplate(req, res) {
  ...
  (0, _magicLog2.default)(libName + ' Rendering Page: ' + page + ' with template ' + template);
...
var page = exports.page = function page(req, res) {
  ...
  _magicLog2.default.success('db.pages is set');
  ...
  db.pages.findOne(pageQuery, function (err, page) {
    if (err) {
      return _magicLog2.default.error(err);
    }
